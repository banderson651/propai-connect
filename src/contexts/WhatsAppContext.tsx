...
interface WhatsAppContextType {
  ...
  sendMessage: (to: string, message: string, templateName?: string, parameters?: any[]) => Promise<boolean>;
...
export const WhatsAppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendMessage = async (to: string, message: string, templateName?: string, parameters?: any[]): Promise<boolean> => {
    try {
      ...
      const response = await fetch('/api/whatsapp/send', {
        ...
        body: JSON.stringify({
          to,
          template: templateName || 'hello_world', // fallback to a default template
          parameters: parameters || [{ type: 'text', text: message }],
        }),
      });
