...
import { parseCSV, parseExcel, getColumnPreviews, ImportMapping, ColumnPreview, ImportProgress, createImportRecord, saveColumnMappings, processImport } from '@/services/contactImportService';
...
export const ImportContacts: React.FC = () => {
    ...
    try {
      ...
      parsedData[0].forEach(header => {
        ...
        const matchedField = fieldOptions.find(field => {
          return field.label.toLowerCase().includes(normalizedHeader) || normalizedHeader.includes(field.value);
        });
        if (matchedField) {
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return <div className="py-8 flex flex-col items-center">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-12 flex flex-col items-center justify-center" onDragOver={handleDragOver} onDrop={handleDrop}>
              {isUploading ? <>
                  ...
                  <Input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls" onChange={handleFileChange} className="hidden" id="file-upload" />
        ...
        return <div className="py-4">
            ...
            <div className="max-h-[400px] overflow-y-auto mb-4">
              <Table>
                ...
                <TableBody>
                  {columnPreviews.map(column => <TableRow key={column.name}>
                      <TableCell className="font-medium">{column.name}</TableCell>
                      <TableCell>
                        <Select value={mappings.find(m => m.sourceColumn === column.name)?.targetField || "none"} onValueChange={value => handleUpdateMapping(column.name, value === "none" ? "" : value)}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select field..." />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="none">-- Do not import --</SelectItem>
                            {fieldOptions.map(option => <SelectItem key={option.value} value={option.value} disabled={mappings.some(m => m.targetField === option.value && m.sourceColumn !== column.name &&
                      ...
                      <TableCell>
                        <div className="text-sm text-gray-500">
                          {column.values.map((value, idx) => <div key={idx} className="truncate max-w-[200px]">
      ...
      case 3:
        return <div className="py-6">
            ...
            {progress && <div className="space-y-6">
                ...
                {progress.errors.length > 0 && <Alert variant="destructive">
                    <AlertCircle className="h-4 w-4" />
                    <AlertTitle>Import Errors</AlertTitle>
                    <AlertDescription>
                      <div className="max-h-32 overflow-y-auto text-sm mt-2">
                        {progress.errors.map((error, idx) => <div key={idx}>{error}</div>)}
                      </div>
                    </AlertDescription>
                  </Alert>}
                
                {!isProcessing && <div className="flex justify-center">
            ...
            {!progress && <div className="flex justify-center items-center py-12">
  ...
  return <Dialog open={isOpen} onOpenChange={open => {
    ...
    if (!open) {
      ...
      <DialogTrigger asChild>
        <Button variant="outline" onClick={() => setIsOpen(true)} className="bg-white">
