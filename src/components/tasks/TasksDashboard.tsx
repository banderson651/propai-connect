  ...
  return <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-3 gap-2 mb-4">
          <button onClick={() => setStatusView('todo')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'todo' ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-700'}`}>
            <Clock className="h-4 w-4 mr-1" />
            <span className="text-sm font-medium">To Do ({todoCount})</span>
          </button>
          <button onClick={() => setStatusView('in-progress')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'in-progress' ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-700'}`}>
            <PlayCircle className="h-4 w-4 mr-1" />
            <span className="text-sm font-medium">In Progress ({inProgressCount})</span>
          </button>
          <button onClick={() => setStatusView('completed')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'completed' ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-700'}`}>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <button onClick={() => setStatusView('due-today')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'due-today' ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-700'}`}>
            <span className="text-sm font-medium">Today ({dueTodayCount})</span>
          </button>
          <button onClick={() => setStatusView('due-tomorrow')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'due-tomorrow' ? 'bg-amber-50 text-amber-700' : 'bg-gray-50 text-gray-700'}`}>
            <span className="text-sm font-medium">Tomorrow ({dueTomorrowCount})</span>
          </button>
          <button onClick={() => setStatusView('overdue')} className={`flex items-center justify-center p-2 rounded-md ${statusView === 'overdue' ? 'bg-red-50 text-red-700' : 'bg-gray-50 text-gray-700'} ${overdueCount > 0 ? 'font-semibold' : ''}`}>
        ...
        {isLoading ? <div className="text-center py-4">
          ...
          </div> : filteredTasks.length === 0 ? <div className="text-center py-4">
            <p className="text-gray-500">No tasks found in this category.</p>
          </div> : <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
            {filteredTasks.slice(0, 5).map(task => <Link key={task.id} to={`/tasks?id=${task.id}`} className="block p-3 hover:bg-gray-50 rounded-md border transition-colors">
                <div className="flex justify-between items-start">
                  <h3 className="text-sm font-medium line-clamp-1">{task.title}</h3>
                  {task.priority === 'high' || task.priority === 'urgent' ? <div className="bg-red-50 text-xs text-red-700 px-1.5 py-0.5 rounded">
                      {task.priority}
                    </div> : null}
                </div>
                {task.dueDate && <div className="text-xs text-gray-500 mt-1">
            ...
            {filteredTasks.length > 5 && <div className="text-center pt-2">
